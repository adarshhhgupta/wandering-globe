...
export function App() {
  ...
  const [savedTrips, setSavedTrips] = useLocalStorage<TripResult[]>('wandering_globe_saved_trips', []);
  const [clientApiKey, setClientApiKey] = useLocalStorage<string>('wandering_globe_api_key', '');
  ...
  const handleRefineTrip = (refinementPrompt: string) => {
  ...
  const handleSaveTrip = (currentTrip: any) => {
    ...
    if (!exists) {
      ...
      setSavedTrips((prev: TripResult[]) => [tripWithId, ...prev]);
    }
  };
  ...
  const handleDeleteSavedTrip = (tripId: string) => {
    setSavedTrips((prev: TripResult[]) => prev.filter((t) => t.id !== tripId));
  };
  ...
  // Load a saved trip
  const handleLoadSavedTrip = (savedTrip: any) => {
    setTrip(savedTrip);
  };
