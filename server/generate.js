 ...
 * @param {string} prompt - Free-form travel prompt
 * @param {string} [apiKeyOverride] - Optional client-supplied key override
 * @returns {Promise<Object>} Validated structured trip object
 */
export async function generate(prompt, apiKeyOverride = null) {
  return generateTripWithGroq(prompt, apiKeyOverride);
}

/**
 * Refines an existing itinerary given a natural language modification.
 * @param {Object} currentTrip - Active itinerary state
 * @param {string} refinementPrompt - Natural language change
 * @param {string} [apiKeyOverride]
 * @returns {Promise<Object>} Updated structured trip object
 */
export async function refine(currentTrip, refinementPrompt, apiKeyOverride = null) {
